import React from "react"
import {NavLink} from "react-router-dom";
import {SocialIcon} from "react-social-icons";
export default function NavBar(){
    return (
        <header className="bg-red-700">
            <div className= "container mx-auto flex justify-between">
                <nav className ="flex">
                    <NavLink to ="/"
                        exact
                        activeClassName="text-white"
                        className= "inflex-flex items-center py-10 px-4 mr-4 text-white hover:text-green-700 text-5xl font-bold cursive tracking widest">
                        Nazeeya Azeem
                    </NavLink>
                    <NavLink to="/post" 
                     className="inline-flex items-center py-3 px-3 my-6 text-red-200 hover:text-green-700 text-lg "
                     activeClassName="text-white bg-red-600"
                     >
                        Blog Posts
                    </NavLink>
                    <NavLink to="/project" 
                     className="inline-flex items-center py-3 px-3 my-6 text-red-200 hover:text-green-700 text-lg "
                     activeClassName="text-white bg-red-600"
                     >
                        Projects
                    </NavLink>
                    <NavLink to="/resume" 
                     className="inline-flex items-center py-3 px-3 my-6 text-red-200 hover:text-green-700 text-lg "
                     activeClassName="text-white bg-red-600"
                     >
                        Resume
                    </NavLink>
                    <NavLink to="/about" 
                     className="inline-flex items-center py-3 px-3 my-6 text-red-200 hover:text-green-700 text-lg "
                     activeClassName="text-white bg-red-600"
                     >
                        About Me!
                    </NavLink>
                </nav>
                <div className="inline-flex py-3 px-3 my-6 ">
                   <SocialIcon url="https://linkedin.com/in/nazeeyaazeem/" className="mr-4" target="_blank" fgColor="#fff" style={{height: 55, width: 55}}/>
                   <SocialIcon url="https://github.com/naz2001" className="mr-4" target="_blank" fgColor="#fff" style={{height: 55, width: 55}}/> 
                </div>
            </div>
        </header>
    )
}
